...
def softmax_output_dice(output, target):
    ...
    # whole (WT):label1+label2+label4
    o = output > 0; t = target > 0 # ce
    ret += dice_score(o, t),
    # core (TC):label1+label4
    o = (output == 1) | (output == 3)
    t = (target == 1) | (target == 4)
    ret += dice_score(o, t),
    # active (ET):label4
...
def validate_softmax(
        ...
        verbose=False,  # If True, print more infomation of the debuging output
    ...
    for i, data in enumerate(valid_loader):
        ...
        if valid_in_train:
            ...
            x, target = data  # x.shape:[1, 240, 240, 155, 4] , target.shape:[1, 240, 240, 155]
